  ...
  const getNavItems = () => {
    ...
    if (isAdmin || isVyapaarOps || isVyapaarFinance) {
      items.push(
        ...
        { label: 'Internal Task Categories', icon: ListTodo, path: '/internal-task-categories', roles: ADMIN_ROLES },
        { label: 'Tax Rates', icon: Percent, path: '/tax-rates', roles: ADMIN_ROLES },
    ...
    const groups = [
      ...
      },
      {
        ...
        items: [
          ...
          has('/internal-task-categories') && byPath['/internal-task-categories'],
          has('/tax-rates') && byPath['/tax-rates'],
