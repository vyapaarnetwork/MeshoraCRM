...
async def create_notification(user_id: str, notification_type: str, title: str, message: str, lead_id: str = None, data: dict = None, commercial_id: str = None):
    ...
    try:
        ...
        if user and user.get('is_active', True) and user.get('email'):
            ...
            # Phase 34.7.3 — global kill-switch: skip emails to customer-role users when admin disables it
            if user.get('role') == UserRole.CUSTOMER.value:
                try:
                    setting = await db.system_settings.find_one({"key": "send_emails_to_customers"}, {"_id": 0})
...
# ==================== Phase 34.7.3 — System Settings (global toggles) ====================
...
async def get_system_setting(key: str, current_user: dict = Depends(get_current_user)):
    """Phase 34.7.3 — read a single system-wide setting. Anyone authenticated can read; only admins write."""
...
async def put_system_setting(key: str, body: SystemSettingPut, current_user: dict = Depends(get_current_user)):
    ...
    await db.system_settings.update_one(
        {"key": key},
        {"$set": {"key": key, "value": body.value, "updated_at": now, "updated_by": current_user['id']}},
        upsert=True,
    )
    return {"key": key, "value": body.value, "updated_at": now}


# Phase 40.1 refactor: Lead Views (LeadViewCreate/Update + 4 CRUD routes) moved to
# routers/lead_views.py. The router is mounted near the bottom of this file.
...
# Phase 40.1 refactor — extract lead-views + lead AI routes into focused modules
from routers.lead_views import router as lead_views_router  # noqa: E402
api_router.include_router(lead_views_router)
from routers.lead_ai import router as lead_ai_router  # noqa: E402
