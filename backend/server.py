...
# ==================== Phase 34.7.3 — Lead Views (saved filter presets) ====================
...
async def create_lead_view(body: LeadViewCreate, current_user: dict = Depends(get_current_user)):
    ...
    if doc['is_default']:
        await db.lead_views.update_many({"user_id": current_user['id'], "is_default": True}, {"$set": {"is_default": False}})
    await db.lead_views.insert_one(doc)
    return {k: v for k, v in doc.items() if k != "_id"}


@api_router.patch("/lead-views/{view_id}")
...
@api_router.delete("/lead-views/{view_id}")
