...
- [x] `CommercialDetail.jsx` reads `?tab=` and `?event=` URL params: auto-selects the **Invoice Files** tab and passes `initialRevenueEventId` to `CommercialInvoiceUploads`.
- [x] `CommercialInvoiceUploads.jsx` — when `initialRevenueEventId` is set, pre-selects the dropdown on the **Vyapaar Commission** section, scrolls it into view, and renders an amber `ring-2` highlight.
- [x] **Verified end-to-end**: curl returned 1 pending event (60d overdue) for admin, 403 for customer. Playwright smoke confirmed card → click → deep-link → highlighted section.
